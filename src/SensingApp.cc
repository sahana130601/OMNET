...
string SensingApp::getSensorValues()
{
    ...
    for (int i = 0; i < sensedDataTypesList.size(); i++) {
        ...
        } else if (sensedDataTypesList[i] == "motion") {
            valint = par("motion");
            snprintf(buffer, sizeof(buffer), "%d", valint);
            sensorValuesStr = sensorValuesStr + string("motion:") + string(buffer) + string(":");

        } else if (sensedDataTypesList[i] == "lightIntensity") {
            valdouble = par("lightIntensity");
            snprintf(buffer, sizeof(buffer), "%.2f", valdouble);
            sensorValuesStr = sensorValuesStr + string("lightIntensity:") + string(buffer) + string(":lux");
